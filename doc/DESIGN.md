...
- **좌측**: `총 N건` → `|`(`.toolbar-sep`) → `새로고침`(`.toolbar-refresh` — 테두리 없는 텍스트형 버튼, `i-lucide-refresh-cw`).
...
ⓑⓒ .list-card              흰 카드 · 1px line · r-lg · overflow hidden
   ├─ ⓑ .list-toolbar       액션 영역 — 카드 "안" 최상단 (border-bottom 1px)
   │     좌: 총 N개 · | · 새로고침   /   우: 페이지별 액션 버튼
   └─ ⓒ table.table         테이블 본체 (가로 스크롤 필요 시 .list-table)
```

- ⓑ 액션 영역 구성·문구·`.toolbar-refresh`·`.toolbar-sep`는 A와 동일.
...
ⓑⓒ .list-card              흰 카드 · 1px line · r-lg · overflow hidden
   ├─ ⓑ .list-toolbar       액션 영역 — 카드 "안" 최상단 (border-bottom 1px)
   │     좌: 총 N개 · | · 새로고침 · 검색란   /   우: 페이지별 액션 버튼
   └─ ⓒ table.table         테이블 본체 (가로 스크롤 필요 시 .list-table)
```

- 검색란은 좌측 그룹의 `새로고침` 다음에 둔다. **너비 260px 고정**, 돋보기 아이콘은 입력칸 오른쪽, 높이는 `.btn-sm`과 동일한 **28px**.
- 액션 영역 좌/우 구성·문구·`.toolbar-refresh`·`.toolbar-sep`는 A·B와 동일.
