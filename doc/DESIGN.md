...
| 부 | 내용 | 적용 |
| --- | --- | --- |
| **Part A. 공용 디자인 코어** | 색 토큰(회색·상태색)·타이포·간격·기본 컴포넌트 패턴·접근성·톤 | 사용자단 + 관리자단 양쪽 |
| **Part B. 사용자단 전용** | 사용자단 브랜드 컬러·1200px 컨테이너·GNB/Footer·발송 페이지 아키텍처 | 이 레포(`malgn-noti`)만 |

> 관리자단은 Part A를 그대로 따르되 자신의 Part B(별도 문서 `malgn-noti-admin/doc/DESIGN-ADMIN.md` 예정)를 추가합니다. 두 문서의 Part A는 동일 값을 유지해야 하며, 차이가 생기면 코어 토큰을 양쪽 동시에 갱신합니다.

---

# Part A. 공용 디자인 코어

> 사용자단·관리자단 모두 이 절을 토대로 작업합니다. 여기서 정의되지 않은 브랜드 색·레이아웃 폭·페이지 패턴은 각 콘솔의 Part B에서 별도 정의합니다.
...
| **절제된 색** | 회색 11단으로 90%, 브랜드·상태 색은 강조에만. |
| **한국어 우선** | Noto Sans KR · letter-spacing 음수로 한글 가독성 우선. 영어는 보조. |
| **컴포넌트 재사용** | Nuxt UI(`U*`) 우선 → 부족하면 `App*` 자체 컴포넌트. 페이지마다 새 디자인 만들지 않음. |
| **접근성 보장** | WCAG AA 대비, 키보드 조작, 포커스 표시는 모든 콘솔의 비협상 기준. |

> **사용자단 특수 원칙**(시안 일치 등)은 [§B1](#b1-사용자단-디자인-원칙) 참조.

---

## A2. 컬러 시스템 (공용)

### A2.1 회색 11단 (Tailwind gray와 동일)
...
CSS 변수는 [`app/assets/css/main.css`](../app/assets/css/main.css) `:root`에 정의.

### A2.2 상태색 (Nuxt UI 토큰)

| 의미 | Nuxt UI 토큰 | 사용 |
| --- | --- | --- |
| 성공 | `color="success"` | 발송 완료, 저장 완료 |
| 경고 | `color="warning"` | 점검, 한도 임박, 미검증 도메인 |
| 위험 | `color="error"` | 삭제, 발송 실패, 위험 액션 |
| 정보 | `color="primary"` | 일반 안내(브랜드 primary와 자동 연동) |

### A2.3 사용 규칙

- 색은 **항상 CSS 변수 또는 Nuxt UI 토큰으로**. `bg-[#6366f1]` 같은 임의값 금지.
- 텍스트는 90%가 `--gray-700 ~ --gray-900` 안에서 결정.
- **대비 4.5:1 이상** (WCAG AA). 본문 위 회색 텍스트는 `--gray-500`까지가 한계.
- 위험·삭제는 `color="error"`, 경고는 `color="warning"`, 성공은 `color="success"`.
- 브랜드 보조색(sky·indigo pill 등)은 각 콘솔의 Part B에서 정의.
...
> 관리자단은 정보 밀도가 더 높아 Display/H1을 축소해 사용할 수 있습니다(예: H1 18–20px). 구체값은 관리자단 Part B에서 정의.
...
> 사용자단은 8px 기본, 관리자단은 정보 밀도를 위해 4px 기본을 사용할 수 있습니다.

---

## A5. 라운드 · 그림자 · 보더 (공용)

### A5.1 라운드 (border-radius)
...
> 추가 라운드(pill 999px, lg 16px 등)는 각 콘솔의 Part B에서 정의.
...
> 브랜드 특수 그림자(예: 사용자단 `brand-glow`)는 각 콘솔의 Part B에서 정의.
...
| 입력 focus | `var(--primary-color)` | 1px (+ ring 2px) | solid |
| 드롭다운 divider (강조 X) | `--gray-300` | 1px | **dashed** |
| 위험 영역 | `color="error"` 톤 | 1px (+ ring) | solid |

---

## A6. 공용 컴포넌트 패턴

### A6.1 본문 버튼 (Nuxt UI `UButton`)
...
> 브랜드 특수 버튼(예: 사용자단 utility bar의 Pill 버튼)은 각 콘솔의 Part B 참조.
...
| **Bootstrap Icons** | `i-bi-*` | 시안 매칭 — 채널 카드, 시안에서 가져온 페이지 (사용자단 위주) |
...
- 강조 (브랜드): 각 콘솔의 브랜드색 — 사용자단은 `var(--color-indigo)`
...
| `lg` | 1024px+ | **데스크톱 기준**: 사이드/탑바 펼침, 2단 콘텐츠 적용 |
| `xl` | 1280px+ | 패딩 확대, 일부 큰 컴포넌트 |

> 1024px 미만에서의 GNB·utility bar·2단 콘텐츠 변화 규칙은 각 콘솔의 Part B에서 정의(사용자단 → [§B6](#b6-사용자단-반응형-변화)).
...
> 관리자단은 내부 용어·영문 약어 노출을 일부 허용합니다(상세는 관리자단 Part B).

### A10.2 빈 상태 / 오류 톤 (공용 베이스)
...
| 발송 한도 초과 (사용자단) | "이번 달 발송 한도(SMS 5,000건)를 초과했습니다." | "Rate limit exceeded" |
...
1. **시안에 동일/유사 패턴이 있나?** → 있으면 시안 base.css 참고해서 토큰 그대로 차용. (사용자단 시안 → [§B7](#b7-시안-매핑))
2. **Nuxt UI에 있나?** → 있으면 그것을 우선 사용 (`UModal`, `UTable` 등).
3. **공통 패턴인가?** → 공통이면 `App*` 컴포넌트로 추출 (예: `AppConfirmDialog`, `AppFilterBar`).
4. **새 색이 필요한가?** → 가능하면 추가하지 말고 기존 토큰으로. 정말 필요하면 `main.css`의 `:root`에 추가하고 본 문서 [§A2](#a2-컬러-시스템-공용) 또는 해당 콘솔 Part B에 등록. **양쪽 콘솔에 공통으로 쓸 색이면 Part A에**, **한쪽만이면 그 콘솔의 Part B에**.
...
> 이 절은 사용자단(`malgn-noti`) 콘솔에만 적용됩니다. 관리자단은 별도 Part B(예정 `malgn-noti-admin/doc/DESIGN-ADMIN.md`)를 따릅니다.
...
| **브랜드 신뢰감** | 고객사 사용자 대상이므로 여백·라운드·미세 그림자로 신뢰감 있는 톤 유지. 정보 밀도보다 가독성 우선. |
| **전환 유도** | 충전·회원가입·문의 같은 핵심 액션은 utility bar의 컬러 Pill로 상시 노출. |
...
Tailwind `indigo-500/600`과 동일. Nuxt UI의 `primary` 색상 토큰과 자동 연동([`app.config.ts`](../app/app.config.ts)에서 `primary: 'indigo'`).

### B2.2 브랜드 보조

| 토큰 | 값 | 사용처 | 시각 |
| --- | --- | --- | --- |
| `--color-sky-soft` | `#E5F4FE` | "문의" pill 배경, 팁 카드 배경, 표 행 hover/selected | 연한 하늘 |
| `--color-sky` | `#4FC0FC` | 사용자 아바타 배경 | 밝은 하늘 |
| `--color-sky-vivid` | `#027CFA` | "충전" pill 배경, 강조 링크, 모달 확인 버튼 | 진한 파랑 |
...
- 위험·삭제 → Nuxt UI `color="error"` ([§A2.2](#a22-상태색-nuxt-ui-토큰))
- 모달 확인 버튼 → `--color-sky-vivid` 톤(시안 그라데이션). 본문 액션의 primary indigo와 구별.
...
[§A6](#a6-공용-컴포넌트-패턴)의 공용 패턴 외에 사용자단에만 적용되는 변형.

### B4.1 Pill 버튼 (utility bar)
...
표 행 hover/selected 시 `--color-sky-soft` (#E5F4FE) 배경 + `--color-sky-vivid` 1px 보더. 표 행 hover는 일반 `--gray-50`이 아닌 sky 톤 사용 (시안 일관).
...
### B5.2 GNB ([AppGnb.vue](../app/components/AppGnb.vue))
...
### B5.3 Footer ([AppFooter.vue](../app/components/AppFooter.vue))
...
[§A8](#a8-반응형-브레이크포인트)의 공용 브레이크포인트 위에 사용자단 특화 변화를 추가:
...
#### B8.3.2 알림톡 메타 예시 (점진적 disclosure + 메시지 유형 baked)
...
const KAKAO: ChannelMeta = {
  ...
  body: {
    ...
    variants: {
      ...
      // channelAdd, auth 등 추후 추가 — §B8.8 TBD-4
    },
  },
  substitution: 'individual-and-common',
  prerequisites: {
...
시안 확인 또는 정책 결정이 필요한 항목들. 결정될 때마다 §B8 본문에 반영하고 이 절에서 제거.
...
| 2026-05-13 | 발송 페이지 아키텍처 신설 — 6채널+Flow 시안 분석으로 5-카드 골격 / ChannelMeta 타입 / App* 컴포넌트 인벤토리 / 모달 카탈로그 / 구현 로드맵 정본화 | — |
| 2026-05-13 | 정밀 분석 반영 — 점진적 disclosure(prerequisites) / body sub-mode(variantBy·variants·BodyVariant) / 치환자 패널을 모든 템플릿 채널 공통으로 격상 / templateLockedFields 매트릭스 / `AppRecipientTable` 1순위 추가 / `AppFileUploader`에 ruleset prop / 모달 컨펌 sky-vivid 명시 / TBD 6항목 도입 | — |
...
| 2026-05-14 | 문서 구조 재편 — **Part A(공용 디자인 코어)** / **Part B(사용자단 전용)** 두 부분으로 분리. 관리자단(`malgn-noti-admin`) 디자인 가이드가 Part A를 공유하도록 토대 정비. 내용 무손실, 섹션 재배치만. | — |
