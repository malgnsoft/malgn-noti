...
`.modal`(r-lg, shadow-modal, slideUp 180ms) — header/body/footer. footer 확인 버튼은 **`ink-900`**(`.btn-sky`도 ink-900로 통일, 구 sky-vivid 폐기). 모바일 메뉴는 좌측 `.drawer`(280px, slideRight). 팝업·다이얼로그는 모두 자체 `AppModal` 기반(`AppConfirmDialog`/`App*Dialog`) — `USlideover`는 사용하지 않으며 모바일 GNB 드로어에만 한정.
