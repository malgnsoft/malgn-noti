...
| [2026-06-01](./history.20260601.md) | WBS 정본화 — `doc/WBS.md` 신규(5 단계 가중치·Step 1~5 작업 내역·알려진 한계, ~220 라인) + 사용자단 `/wbs` 공개 라이브 카탈로그(`malgn-helper/doc/WBS.md` 양식 + `malgn-helper-pms/pages/wbs.vue` Notion soft SaaS 디자인 차용 — Hero stats·단계별 진행률 오버뷰·Stage 상세·그룹별 작업 카드·상태 칩·외부 링크 + `layout: blank`·`auth: false`, ~650 라인). Step 5(서비스 개발)는 원본 채널·도메인 단위 항목(대부분 0%)을 실제 진행에 맞춰 `5-1 설계 및 준비`(7) / `5-2 API 서버`(16) / `5-3 사용자단 화면`(15) / `5-4 관리자단 화면`(13) / `5-5 통합·배포`(7)의 5 그룹 58 작업으로 재정렬 — 사용자단 화면 13 ✅ + 시스템 페이지 🟢 + 백엔드 연동 ⚪ / API 9 ✅ + 3 🟢 + 4 ⚪ / 관리자단 셸·기획 ✅ + P0~P2 화면 ⚪ / 배포 ✅ 3 + ⛔ DDL 1. 가중평균 약 45%. Cloudflare Pages 배포 #47 (alias `0ecc825e.malgn-noti.pages.dev`). |
